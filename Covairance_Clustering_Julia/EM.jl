...
function W_em_base(
    ...
    log_new = 0#W_normal_likelihood(X, class, mix)
    log_old = -Inf
    steps = 0
    params_sum_new = 0
    params_sum_old = -1
        ...
        for b in 1:B
            for i in 1:n
                for k in 1:K
                    ...
                    # z[i, b] += mix.Pi[k] * exp(sum(logpdf(N,
                    #     transpose(em_results.X[em_results.class.==b, :]))) +
                    #                            logpdf(N, X[i, :]))
                end
...
# yy = generate_random_wishart_mixture(3, 5)

# uu = uniform_sampling_closed_set(yy, 1000, 5, 0, 5)

# # # # test = rand(1:length(uu.class), 20)

# em = W_em(uu.X, uu.class, K=[3]; M=100, initalization="hierarchical",is_diag=false, return_dict=false)
...
# # ff = TypeWishartToDict(em[1])

# # ff[:ID]

# # em[1].imputed_cov_REML[1]
# # # em.mix.Pi

# mean(parse.(Int, predict_old(em[1], uu.X[in.(1:500, Ref(test)) .== 1, :])) .== uu.class[in.(1:500, Ref(test)) .== 1])

# mean(parse.(Int, predict(em[1], uu.X[in.(1:500, Ref(test)) .== 1, :])) .== uu.class[in.(1:500, Ref(test)) .== 1])
...
# mean(pp .== string.(uu.class))


# # ProfileView.@profview em = W_em(uu.X, uu.class, 5, computational=false, is_diag=false)

# # em = W_em(uu.X, uu.class, 5, computational=false, is_diag=true)
...
# class = ["a", "a","a", "a","a", "a", "c", "c","c", "c","c", "c", "d", "d"]
...
#     class_map[i] = classes[findall(class[i] .== classes)[1]]

# end

# values(class_map)

# [classes[i] for i in values(class_map)]

# class_map = Dict()
# for i in class
#     class_map[i] = findall(i .== classes)[1]
# end

# rr = Dict([(i=> findall(i .== classes)[1]) for i in classes])
